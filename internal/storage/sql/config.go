...
func (s *SQLStore) ListConfigs(ctx context.Context) ([]*model.Config, error) {
	...
	query := `
			SELECT c.id, c.name, c.url, c.priority, c.channel_type, c.enabled,
			       c.cooldown_until, c.cooldown_duration_ms, c.daily_cost_limit, c.proxy_url,
	...
	err := s.WithTransaction(ctx, func(tx *sql.Tx) error {
		if id == 0 {
			...
			res, err := tx.ExecContext(ctx, `
				INSERT INTO channels(name, url, priority, channel_type, enabled, daily_cost_limit, proxy_url, created_at, updated_at)
				VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)
			`, c.Name, c.URL, c.Priority, channelType,
				boolToInt(c.Enabled), c.DailyCostLimit, c.ProxyURL, nowUnix, nowUnix)
			if err != nil {
				return err
			}
			...
			if s.IsSQLite() {
				_, err := tx.ExecContext(ctx, `
					INSERT INTO channels(id, name, url, priority, channel_type, enabled, daily_cost_limit, proxy_url, created_at, updated_at)
					VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
func (s *SQLStore) UpdateConfig(ctx context.Context, id int64, upd *model.Config) (*model.Config, error) {
	...
	err := s.WithTransaction(ctx, func(tx *sql.Tx) error {
		...
		_, err := tx.ExecContext(ctx, `
			UPDATE channels
			SET name=?, url=?, priority=?, channel_type=?, enabled=?, daily_cost_limit=?, proxy_url=?, updated_at=?
			WHERE id=?
		`, name, url, upd.Priority, channelType,
			boolToInt(upd.Enabled), upd.DailyCostLimit, upd.ProxyURL, updatedAtUnix, id)
		if err != nil {
			return err
		}
