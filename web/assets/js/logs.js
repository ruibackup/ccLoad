    ...
    const PROMPT_FULLSCREEN_STATE_KEY = 'ccload.logs.promptFullscreenState';
    ...
    let promptFullscreenDragState = null;
    let promptFullscreenInteractionsInited = false;
    ...
    function applyFilter() {
      ...
      load();
      initPromptFullscreenInteractions();
    }
    ...
    document.addEventListener('DOMContentLoaded', async function() {
      ...
      document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
          const promptFullscreenModal = document.getElementById('promptFullscreenModal');
          if (promptFullscreenModal && promptFullscreenModal.classList.contains('show')) {
      ...
      const promptFullscreenModal = document.getElementById('promptFullscreenModal');
      if (promptFullscreenModal) {
        promptFullscreenModal.addEventListener('click', (e) => {
          if (e.target === promptFullscreenModal) {
            closePromptFullscreen();
          }
        });

        const modalContent = getPromptFullscreenModalContent();
        if (modalContent) {
          modalContent.addEventListener('mouseup', () => {
            if (!promptFullscreenModal.classList.contains('show')) return;
            persistPromptFullscreenState();
          });
        }
      }
    ...
    // ========== 日志详情功能 ==========
    let currentLogsData = []; // 缓存当前页日志数据
    let logDetailPromptStore = []; // 日志详情：可读视图消息缓存（用于全屏查看）
    ...
    function loadPromptFullscreenState() {
      try {
        const raw = localStorage.getItem(PROMPT_FULLSCREEN_STATE_KEY);
        if (!raw) return null;
    ...
    function savePromptFullscreenState(state) {
      try {
        localStorage.setItem(PROMPT_FULLSCREEN_STATE_KEY, JSON.stringify(state));
      } catch (_) {
        // 忽略存储失败（如隐私模式）
      }
    }

    function getPromptFullscreenModalContent() {
      return document.querySelector('#promptFullscreenModal .prompt-fullscreen-modal');
    }

    function clampPromptFullscreenRect(modalContent, left, top) {
      const maxLeft = Math.max(0, window.innerWidth - modalContent.offsetWidth);
      const maxTop = Math.max(0, window.innerHeight - modalContent.offsetHeight);
      return {
    ...
    function applyPromptFullscreenState() {
      const modalContent = getPromptFullscreenModalContent();
      ...
      const state = loadPromptFullscreenState();
      if (!state) return;

      if (Number.isFinite(state.width) && state.width > 400) {
        modalContent.style.width = `${Math.min(state.width, window.innerWidth - 24)}px`;
      }
      if (Number.isFinite(state.height) && state.height > 300) {
        modalContent.style.height = `${Math.min(state.height, window.innerHeight - 24)}px`;
      }

      if (Number.isFinite(state.left) && Number.isFinite(state.top)) {
        const clamped = clampPromptFullscreenRect(modalContent, state.left, state.top);
        modalContent.style.left = `${clamped.left}px`;
        modalContent.style.top = `${clamped.top}px`;
        modalContent.style.transform = 'none';
      }
    }

    function persistPromptFullscreenState() {
      const modalContent = getPromptFullscreenModalContent();
      if (!modalContent) return;

      const rect = modalContent.getBoundingClientRect();
      const usesFixedPosition = modalContent.style.transform === 'none';
      const state = {
        width: Math.round(rect.width),
        height: Math.round(rect.height),
        left: usesFixedPosition ? Math.round(rect.left) : null,
        top: usesFixedPosition ? Math.round(rect.top) : null
      };
      savePromptFullscreenState(state);
    }

    function initPromptFullscreenInteractions() {
      if (promptFullscreenInteractionsInited) return;

      const modalEl = document.getElementById('promptFullscreenModal');
      const modalContent = getPromptFullscreenModalContent();
      const headerEl = modalContent?.querySelector('.modal-header');
      if (!modalEl || !modalContent || !headerEl) return;

      promptFullscreenInteractionsInited = true;
      headerEl.classList.add('prompt-fullscreen-drag-handle');
      ...
      headerEl.addEventListener('mousedown', (e) => {
        ...
        promptFullscreenDragState = {
          offsetX: e.clientX - rect.left,
          offsetY: e.clientY - rect.top
        };

        document.body.classList.add('prompt-fullscreen-dragging');
        e.preventDefault();
      });

      window.addEventListener('mousemove', (e) => {
        if (!promptFullscreenDragState || !modalEl.classList.contains('show')) return;

        const nextLeft = e.clientX - promptFullscreenDragState.offsetX;
        const nextTop = e.clientY - promptFullscreenDragState.offsetY;
        const clamped = clampPromptFullscreenRect(modalContent, nextLeft, nextTop);
      ...
      window.addEventListener('mouseup', () => {
        if (!promptFullscreenDragState) return;
        promptFullscreenDragState = null;
        document.body.classList.remove('prompt-fullscreen-dragging');
        persistPromptFullscreenState();
      });
    }

    function openPromptFullscreen(msgIdx) {
      if (!Number.isInteger(msgIdx) || msgIdx < 0 || msgIdx >= logDetailPromptStore.length) return;
      const item = logDetailPromptStore[msgIdx];
      if (!item) return;

      initPromptFullscreenInteractions();
      applyPromptFullscreenState();

      const titleEl = document.getElementById('promptFullscreenTitle');
      const contentEl = document.getElementById('promptFullscreenContent');
      const modalEl = document.getElementById('promptFullscreenModal');
      if (!titleEl || !contentEl || !modalEl) return;

      titleEl.textContent = `${item.label} - ${t('logs.fullscreen')}`;
      contentEl.innerHTML = renderSimpleMarkdown(item.text || '');
      modalEl.classList.add('show');
      contentEl.scrollTop = 0;

      const bodyEl = modalEl.querySelector('.prompt-fullscreen-body');
    ...
    // 从content字段提取纯文本（兼容string和array两种格式）
    function extractTextFromContent(content) {
      ...
      for (const block of content) {
        if (typeof block === 'string') {
          ...
          // Anthropic: {type:"text", text:"..."} / Gemini: {text:"..."}
    ...
    function renderChatMessage(role, text) {
      ...
      const msgIdx = logDetailPromptStore.length;
      logDetailPromptStore.push({ role: safeRole, label, text });

      const canFullscreen = safeRole === 'system' || safeRole === 'user';
      const fullscreenBtn = canFullscreen
        ? `<button type="button" class="chat-fullscreen-btn" onclick="openPromptFullscreen(${msgIdx})" title="${escapeHtml(t('logs.fullscreen'))}" aria-label="${escapeHtml(t('logs.fullscreen'))}">
      ...
      return `<div class="chat-message chat-role-${safeRole}">
        <div class="chat-message-header">
          <span class="chat-message-title">${escapeHtml(label)}</span>
          ${fullscreenBtn}
        </div>
    ...
    // 轻量Markdown渲染（安全：先转义HTML，再解析Markdown语法）
    function renderSimpleMarkdown(text) {
      if (!text) return '';

      // 1. 先提取代码块和行内代码，用占位符替换（避免内部被其他规则干扰）
      const codeBlocks = [];
      const inlineCodes = [];
    ...
    function renderReadableRequest(bodyStr) {
      let data;
      try { data = JSON.parse(bodyStr); } catch (e) { return `<div class="code-block"><pre class="json-content">${escapeHtml(bodyStr)}</pre></div>`; }
      if (!data || typeof data !== 'object') return `<div class="code-block"><pre class="json-content">${escapeHtml(bodyStr)}</pre></div>`;

      logDetailPromptStore = [];

      const parts = [];

      // 元信息（model, stream, max_tokens等）
      const metaItems = [];
